...
From this, we introduce some an alternative representation for arithmetic
circuits and give several key definitions, which are needed for reasoning
...
\begin{definition}[Arithmetic Circuit]
  ...
  The means that the value of the circuit can be obtained by computing the value
  of $o$.

  Finally, for all gates $g \in G$ there must exist a path in W from in to o going
  through g,
  since if this was not the case, the gate does not contribute to the output of the
...
visited by the BFS.
This ensures that for any node with index $i$ only
...
which is a full and uniformly distributed over the shares; in this case field elements.
...
the random choices made throughout the protocol.
...
\end{lstlisting}
  ...
  To prove that the predicate holds after computing gate g we prove that it holds
...
views become important for the proof of the ZKBoo protocol.
...
This is necessary to be able to reconstruct the output share $y_{e+2}$.
...
computed by the decomposition there would be no way to conclude the truthiness of \texttt{valid\_view\_op}.
