...
\citet{zkboo}, but to formalise these, we have had to make definitions more
...
Throughout this chapter, use the letter $e$ to denote a challenge expressed as
an integer in \{1,2,3\}. Moreover, we define the views after $3$ to be $1$.
Logically the challenges could be denoted $\mathbb{Z}_{3}$, but we chose the
...
value.
It is clear from this we need to define an evaluation order of the
...
This ordering ensures that for any node given index $i$ will only
depends on computations of gates with index less-than $i$.
...
linear ordering.
However, since the input gate does not perform any computations it only
exist to add the input value to the graph, we exclude it from the list
representation and shift every index one down.

\paragraph{Encoded gates}
An encoded gate is a type, where the type defines the operation the gate computes along
...
a well-defined evaluation order. For specific choices of linear orderings we can
ensure that we are not trying to compute the value of a gate before the all its
...
This representation can then be extended to allows for multiple inputs. If we
want the circuit have $n$ input gates then we reserve the first $n$ entries of
...
In this section formalise the (2,3)-Decomposition of arithmetic
circuits based on the description given in section \ref{subsec:general:arith}.

In its most general form, we define the decomposition as a procedure taking
...
important that the views produced by the decomposition we are currently
...
\begin{proof}
  ...
  We then showing that calling \texttt{compute} and \texttt{simulate} will
...
return $(k_e, k_{e+1} k_{e+2}, w_e, w_{e+1})$;
  ...
  When simulating MULT view $w_{1}$ is constructed by calling \texttt{Upate}
  ...
  respectively.
  Indistinguishability here follows from the fact that \texttt{dinput} in full
  and uniform, and that field from which the shares are sampled from is closed
...
To achieve this use introduce a specialized version of a lemma proven by \citet{Yao},
...
proc ideal(h, e) = {
  ...
  can then apply correctness lemma to both sides, thus making both procedures
  return true.
  ...
  (\ref{def:commitment:perfect-hiding}) which states that two executions of
  \texttt{Commit} with identical state produce indistinguishable commitments.
...
$w'_{i}$. Now, since the two potential openings corresponds to a commitment in message $a$ they should
