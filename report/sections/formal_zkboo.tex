...
An arithmetic circuit in its most general form expresses a function $f$ over some
...
\begin{definition}[Arithmetic Circuit]
  ...
  An Arithmetic circuit is a directed acyclic graph C = (W, G) where W is the internal
...
However, we need to encode W into this list. To do this, we encode the information
...
\todo{What if we want more than one input/output}
...
compute the value of a gate before the all its inputs wires has been assigned
...
into a list called ``state'' at the appropriate index.
...
\todo{explain parameters to eval better?}
\todo{explain simulator wrapper around eval}
...
\begin{lemma}[Decomposition 2-Privacy]
  ...
  \[
    equiv[real \sim simulated : \indis{e, x, c} \land y^{simulated} = \texttt{eval\_{circuit}} \; c \; x^{real} \implies =\{res\}].
  \]
...
return $(k_e, k_{e+1} k_{e+2}, w_e, w_{e+1})$;
  ...
  type, except MULT, will call \texttt{Update}. This equivalent to
  \texttt{compute}, which calls \texttt{Update} directly.
  We, therefore, only
  need to look at the case of MULT.

  When simulating MULT $w_{1}$ is constructed by calling \texttt{compute}
  ...
  specially, the value of $r_{2}$ is used as the share of party $2$.

  Since the randomness is sampled is sampled at the beginning of both procedures
...
together. This is seen in listing \ref{lst:zkboo:alt-soundness}
...
$w'_{i}$, since all these openings corresponds to a commitment in message $a$ then should
