...
gate and then save those choices to some random tapes. These tapes are then
returned. They are used to keep track of the random choices made thoughout the
protocol, such that views can be verified. For most proof the specific values
contained within the random tapes are not imporant, since the random values are
only there to cancel eachother out whilst making the shares look randomly
distributed.
We therefore omit the random tapes for most procedures and proofs in this
section, since they are static? When the random tapes are imporant for the
security we will mentioned how.
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item Write this as program steps like in \cite{certicrypt_sigma}?
      ...
      \end{align*}
    \item Possible due to inlined randomness sampling
    \item If tape has been sampled before the run we cannot use EC's coupling
      functionality to reason about the random choices.
  \end{itemize}
