...
\citet{zkboo}. In order to formalise these, we have had to make definitions more
precise and prove certain predicates about the different procedures of
the protocol in order to prove it secure. These changes have been necessary since crucial
...
graphs which is also used in the original paper by \citet{zkboo} and discuss how
...
It is then possible to define the semantic meaning of the list representation of
...
Based on procedure defined in \autoref{lst:decomp_aux} we then define what
...
\begin{definition}[Correctness of views]
  ...
  their computation \eg\ when computing a MULT gate, then the share used by
...
This can be stated as an invariant on the
...
\end{lemma}
\begin{proof}
  We show that while-loop will never break the invariant given by the
  postcondition.
  We prove this by induction on the number of step iterated through the
  while-loop.
...
This additional restriction on the views become essential for proving the
security of ZKBoo.
...
\texttt{simulate} is the a wrapper around \texttt{simulator\_eval}, which is
...
\begin{lemma}[Decomposition 2-Privacy]
  ...
  \[
    equiv[real \sim simulated : \indis{e, x, c} \land y^{simulated} = \texttt{eval\_{circuit}} \; c \; x^{real} \implies =\{w_{e}, w_{e+1}, y_{e+2}\}].
  \]
...
return $(k_e, k_{e+1} k_{e+2}, w_e, w_{e+1})$;
  ...
  and uniform, and that field, from which the shares are sampled, is closed
  under arithmetic operations.

  Had the randomness been sampled before running the protocol, this would not have been possible,
  since \easycrypt\ can only reason about indistinguishability of sampled values at
...
To achieve this we introduce a specialized version of a lemma proven by \citet{Yao},
...
proc ideal(h, e) = {
  ...
  \texttt{simulate} are indistinguishable, buy only when observing the views
...
are accepting. To verify the views it uses the binding game, which only checks
...
distribution of verifying a commitment and the alternative binding game is identical.
...
\end{lemma}
\begin{proof}
  This follows directly from inlining all procedures and applying our assumptions.
\end{proof}

Consequently, since \texttt{extract\_views} returns true for all calls to
\texttt{verify} we can conclude \validviews{c, w_1, w_2, w_3} and
