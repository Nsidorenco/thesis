...
\paragraph{Outline}
First, in section \ref{sec:arith_circuits} we find workable formalisation of
...
\todo{Thought out this section we assume that all challenges are sampled within the
...
\todo{This section ...}
% We start by stating some of the restriction we have on arithmetic
% circuits. First, arithmetic circuits are only allowed to perform deterministic
We start by stating a number of simplifying assumptions about our arithmetic
circuits. First, we only allow the circuit to have one input value and one
output value.
This assumption could be alleviated by change the definition slightly and
letting the circuit perform operations on tuples of integers rather than just
integers.

To express arbitrary (Arithmetic) computations in a ring of finite integer we
...
For an arithmetic circuit consisting of the aforementioned four gates we recall
the standard definition of circuits as graphs, but accounting for out simplifying alterations:
\begin{definition}[Arithmetic Circuit]
  \label{def:arith_circuit}
  An Arithmetic circuit is a graph C = (W, G, I, o) where W is the internal
  wires between the gates and G is the set of gates within the circuit. Then,
  i is a designated first gate of the circuit and o is the designated output
...
To define the order of evaluation we start by introducing an alternative
...
\begin{definition}[List representation of arithmetic circuits]
  ...
  \[
    C_{L}[i] = Enc(O( G \setminus \{i,o\})[i], W)
  \]
...
We simplify our notion of the computation order and remove all parallel
computations. To achieve this we define a linear order on the gates by perform a
breadth-first numbering of the nodes starting from the input gate i and ending
at o.
...
\begin{definition}[Valid circuit]
  ...
  An arithmetic circuit in list representation $C_{L}$ is valid if:
  \begin{itemize}
...
\begin{definition}[Correctness of views]
  ...
  \todo{Explain that w[0] is the input}
  \begin{equation}
