...
\cite{zkboo}. To formalise this, we utilise our formalisations of
$\Sigma$-Protocols and commitment schemes.
% ZKBoo is a $\Sigma$-Protocol, so we
...
\cite{zkboo}. In order to formalise these, however, we had to make the definitions more
precise and prove certain predicates about the different procedures of
the protocol in order to prove them secure. These changes have been necessary since crucial
...
verification can help us identify pitfalls in informal proofs.
...
Throughout this chapter, we use the letter $e$ to denote a challenge expressed as
an integer in \{1,2,3\}. Moreover, we define the views after $3$ to be $1$.
Logically the challenges could be denoted $\mathbb{Z}_{3}$, but we chose to
follow the notation of the original paper here.

\section{Formalising Arithmetic circuits}
\label{sec:arith_circuits}
Before we can reason about the decomposition of an arithmetic circuit, we need a clear
...
graphs which is also used in the original paper by \cite{zkboo} and discuss how
...
An arithmetic circuit in its general form expresses a function $f$ over some
arbitrary finite field $\mathbb{Z}_{q}$, where
$f : \mathbb{Z}_{q} \rightarrow \mathbb{Z}_{q}$
...
computations.
% Before doing so, however, we start by stating several
...
\begin{definition}[Arithmetic Circuit]
  ...
  An Arithmetic circuit is a directed acyclic graph C = (W, G) where W is the
  wires between the gates and G is the set of gates within the circuit. Then,
  we let $in \in G$ be the first gate of the circuit, \ie\ the input and $o \in G$ be the final
  ...
  through $g$.
  If this was not the case, then the gate could be removed from the graph
  without changing the semantics of the circuit.
\end{definition}

To define the evaluation of the circuit, we would then need to compute the values of the
in-going wires of $o$, but this requires all other wires in the circuit to be
computed first.
Consequently, the value of the out-going wires of a gate can only
be computed if all in-going wires of the gate have already been assigned a
value.
It is clear from this that we need to define an evaluation order of the
...
To define the order of evaluation, we follow the work of \cite{Yao} and
introducing an alternative representation of Arithmetic circuits.
However, before doing so, we first introduce linear orderings and encoded gates:

\paragraph{Linear ordering}
A linear ordering O is a function that applied to G assigns an unique
index to each gate in G.
One example of such a linear ordering is the breadth-first
search (BFS) of a graph.
Here each gate in the graph is labelled according to when the BFS visited the gate/node.
This labelling would start at the input gate and end at the output gate.

A particularly useful property of the linear ordering induced by a BFS, is that a
gate can only be visited if the BFS has already visited all nodes with wires going to that gate.
This ordering ensures that, for any given node, with the index $i$ will only
depend on computations of gates with an index less-than $i$.
...
linear ordering.
However, since the input gate does not perform any computations, except for
adding the input value to the graph, we exclude it from the list
representation and shift every index one down.

\paragraph{Encoded gates}
An encoded gate is a type, where the type defines the operation which the gate computes along
...
the computation.
Here, the tuple $(l,r)$ is the wires of $W$ encoded directly into the gates. The
type definition can been seen in \autoref{lst:gate_types}.
...
\begin{definition}[List representation of arithmetic circuits]
  Given an arithmetic circuit C (\autoref{def:arith_circuit})
  we define the list representation of C by computing a linear ordering, O, of
  G, which assigns each gate in G an unique index.
  ...
  gate.
  Encoded gates contain type information about the gate but also stores the
...
One important aspect of the list representation of circuits is that it gives us
a well-defined evaluation order. For specific linear orderings we can
ensure that we are not trying to compute the value of a gate before all its
inputs wires has been assigned a value. To capture this notion of a valid
evaluation order, we give the following definition:
...
\begin{definition}[Valid circuit]
  ...
  entry $i$ in the list it holds that:
    \begin{itemize}
      ...
      \item the gate corresponding to the input wires of C[i] have index $j$ where $0 \leq j<i$.
    \end{itemize}
\end{definition}
...
It is then possible to define the semantics of the list representation of
a circuit by defining an evaluation function, which can be seen in \autoref{lst:circuit_eval}.
...
to pattern match on the type of the gate as seen in \autoref{lst:circuit_eval}.
...
To maintain the list of computed gates, we define the ``state'', which at index
$i+1$ will store the value for performing gate evaluation of the gate at index $i$

For example, when computing the gate $MULT(i,j)$, we simply look up the valued of index
...
\begin{definition}[State of list representation]
  ...
  Here we recall that the input gate has been removed from the list representation
...
This representation can then be extended to allow for multiple inputs. If we
want the circuit to have $n$ input gates, then we reserve the first $n$ entries of
the state to be inputs. Consequently, we then shift every index in our encoded
gates by $n$. Moreover, if want to support multiple output we need to fix the
ordering such that the $m$ output gates are always computed last. By computing
the output gates last we are ensured that the last $m$ entries of the state will
be the outputs.


We then have that any valid circuit c can be computed to a value y as
...
In this section, we formalise the (2,3)-Decomposition of arithmetic
circuits based on the description given in section \ref{subsec:general:arith}.

In its most general form, we define the decomposition as a procedure
\texttt{compute}, which takes an input three views and three random tapes along
with the circuit it needs to compute. The results of running \texttt{compute} is
then three new views and random tapes, which express the decomposed computation.
More specifically, the decomposition works by incrementally evaluating a gate based on
previously computes views, which yields new shares that can be appended to the
view. We then repeat the process of evaluating a single gate based on the view of evaluating
the previous gate until all gates have been computed.
% This
% overall idea has been captured in the procedure in \autoref{lst:decomp_aux},
...
the entire view. Additionally, \texttt{Update} takes an extra parameter $p$,
which is used to decide which party is computing the share. This is use for the
ADD gate to determine if $p=1$.
Moreover, when calling \texttt{Update} on gate $g$,
...
from each view that has been computed by the aforementioned procedure.
Here we use the evaluation function for arithmetic circuits defined in the previous section,
which tells us that the last computed share is the output share.
More formally the reconstructed output is:
\begin{equation}
  \sum_{i \in \{1,2,3\}} \text{last } w_{i}
\end{equation}

Based on the procedure defined in \autoref{lst:decomp_aux} we then define what
...
\begin{definition}[Correctness of views]
  ...
  \texttt{eval\_circuit\_aux} in \autoref{lst:circuit_eval}.

  Consequently, a share is only valid if it has been produced by the decomposition.
  \eg\ when computing a MULT gate, where party $p$ used the shares
  $w_{p}[i]$ and $w_{p}[j]$ to compute his next share, then the shares used by
  the other parties did indeed match $w_{p}[i]$ and $w_{p}[j]$.
  \begin{equation}
  ...
  To express that the views satisfy the above equations we use the notation
...
Looking at \texttt{compute} we see that it makes three random choices for each
iteration of the while-loop and then save those choices in the random tapes.
...
In most proofs, it is not essential to verify the shares of previously computed decomposition.
Specifically, when considering the correctness and 2-privacy properties it is only
important that the views produced by the decomposition, which we are currently
computing, is going to be valid.
This can be stated as an invariant of the
decomposition rather than verifying it by re-running the decomposition.
...
Last, since \texttt{compute} samples randomness at the time it is needed by the
...
\autoref{lst:compute-fixed} and works precisely like \texttt{compute}
...
\end{lemma}
\begin{proof}
  We show the while-loop will never break the invariant given by the
  postcondition.
  We prove this by induction in the number of step iterated through the
  while-loop.
  Now, since we know by definition of \texttt{Update} that the randomness has no
  influence on the output shares, the proof is trivially true.
\end{proof}
...
\end{lstlisting}
  ...
  To prove that the predicate holds after computing gate g we split the prove
  based on the type of g.
  The proof is then complete by inlining the definition of \texttt{Update}.
\end{proof}
...
In some sense, our work imposes stricter restrictions on the correctness of decomposition than
the proof by \cite{zkboo} since we require that every share computed can be
proven correct by decomposition, while the original proof of the correctness only
requires that the output shares sum to the output of the circuit evaluation.
The original proof does satisfy the definition of MPC correctness, however, if
we do not add this additional restriction then it is not possible
to prove the security of the ZKBoo protocol.
This restriction is implicitly implied by \cite{zkboo} since all parties are
assumed honest. However, we need state to this restriction explicitly since we
otherwise have no way to argue that the validation step described in section
\ref{subsec:general:zkboo} will succeed if it is not a property of the
decomposition that all shares are correctly produced.

\subsection{2-Privacy}
\label{sec:decomp_privacy}
To prove 2-Privacy, we define a simulator capable of producing two views, which are
indistinguishable from two views produced by the decomposition.
The simulator is given by the
procedure \texttt{simulate} and function \texttt{simulator\_eval} in \autoref{lst:zkboo:simulator}.
...
the simulator calls the \texttt{Update} function just like
\texttt{compute}.
...
\texttt{simulate} is a wrapper around \texttt{simulator\_eval}, which is
responsible for calling \texttt{simulator\_eval} on each gate in the circuit and
appending the newly computed shares to the views.
...
Here, \texttt{simulated} is given access to the output of the decomposition as by the definition of privacy of MPC protocols.
...
proc real((c,y) : statement, x : witness, e : challenge) = {
  ...
  $y_{1} = \textbf{last } w_{1}$;
  $y_{2} = \textbf{last } w_{2}$;
  $y_{3} = \textbf{last } w_{3}$;
...
\begin{lemma}[Decomposition 2-Privacy]
  ...
  \[
    equiv[real \sim simulated : \indis{e, x, c} \land y^{simulated} = \texttt{eval\_{circuit}} \; c \; x^{real} \implies \indis{w_{e}, w_{e+1}, y_{e+2}}].
  \]
...
\begin{lemma}
  ...
  \begin{align*}
    equiv[&\texttt{compute} \sim \texttt{simulated} : \; =\!\{c, e, w_{e}, w_{e+1}\} \\
           &\implies =\!\{w_{e}, w_{e+1}\} \land \sum_{i \in \{1,2,3\}} \textbf{last } w^{compute}_{i} = \texttt{eval\_{circuit}} \; c \; x]
  \end{align*}
...
$(w_{1}, w_{2}, w_{3})$ = compute$([g], [x_{1}], [x_{2}], [x_{3}])$;
$(w_{1},w_{2},w_{3})$ = compute$(c', w_{1}, w_{2}, w_{3})$;
return $(w_{1}, w_{2}, w_{3})$
    \end{lstlisting}
    ...
    \begin{subfigure}{ 0.48\textwidth }
    \begin{lstlisting}[mathescape]
$(w_{e}, w_{e+1})$ = simulate$([g], e, [x_{e}], [x_{e+1}])$;
$(w_{e}, w_{e+1})$ = simulate$(c', e, w_{e}, w_{e+1})$;
return $(w_{e}, w_{e+1})$
    \end{lstlisting}
...
\begin{proof}
  ...
  \noindent \textbf{Induction step $c = g::c'$: }
  We then start by splitting the computation of \texttt{compute} and
  \texttt{simulate} into two separate calls; one for computing g and another for
  computing the circuit $c'$ as seen in \autoref{lst:real/simulated}.
  We then show that calling \texttt{compute} and \texttt{simulate} will
  satisfy $\indis{w_{e}, w_{e+1}}$. and that the output shares of \texttt{compute} sums
  to the output.
...
return $(k_e, k_{e+1} k_{e+2}, w_e, w_{e+1})$;
    \end{lstlisting}
  ...
  on which the procedures operate. This can be seen in \autoref{lst:privacy:inlined}.
  We then start by applying the induction hypothesis, which states that \texttt{compute} and \texttt{simulate} called on $c'$ is indistinguishable given that the view from running the procedures on the gate $g$ are indistinguishable.
  The proof is then split based on the value of the challenge:

  \noindent$e=1$ : We then further split the proof based on the type of the
  gate. For every gate except MULT, the computation for \texttt{compute} and
  \texttt{simulate} are the same for parties $e$ and $e+1$.
  This is true, since \texttt{simulator\_{eval}}, when called on every gate
  type, except MULT, will call \texttt{Update}.
  This equivalent to \texttt{compute}, which calls \texttt{Update} directly.
  We, therefore, only need to look at the case of MULT.

  When simulating MULT view $w_{1}$ is constructed by calling \texttt{Update}
  and is therefore trivially indistinguishable. For the construction of
  $w_{2}$ the value is sampled at random by the simulator. More
  specially, the value of $r_{3}$ is used as the share of party $2$. The reason
  why we need to use the value of $r_{3}$ is that $r_{1}$ and $r_{2}$ must be
  sampled equivalently between the two procedures, otherwise $w_{1}$ would not
  be construed correctly. $r_{3}$, however, is not used for any computation
  relating to the output. We are, therefore, free to manipulate the value of it.

  Since the randomness is sampled at the beginning of both procedures
  we can use \easycrypt's coupling functionality to reason about
  indistinguishability of the sampled values.
  In this case, we prove that the computation perform for MULT by
  \texttt{compute} is indistinguishable from the share $r^{simulate}_{3}$:
  \begin{align*}
    (&w_{2}^{compute}[l] \cdot  w_{2}^{compute}[r] \\
     &+ w_{3}^{compute}[l] \cdot w_{2}^{compute}[r] \\
     &+ w_{2}^{compute}[l] \cdot w_{3}^{compute}[r] \\
     &+ r^{compute}_{2} - r^{compute}_{3}) \sim r^{simulate}_{3}.
  \end{align*}
  where $l$ and $r$ are the indexed of the left and right input wire,
  respectively.
  Indistinguishability here follows from the fact that \texttt{dinput} is full
  and uniform, and the field, from which the shares are sampled, is closed
  under algebraic operations.

  From this we can conclude that views $w_{1}$ and $w_{2}$ are indistinguishable
  between the two protocols. Moreover, we have that
  $\sum_{i \in \{1,2,3\}} \textbf{last } w^{compute}_{i} = \texttt{eval\_{circuit}} \; [g] \; x$,
  by the correctness property of \texttt{compute}.
...
For the proof the above lemma we used the fact that randomness is sampled when
needed by the computations. This allowed us to use \easycrypt's coupling
functionality to manipulate the values of the random samplings.
Had the randomness been sampled before running the protocol, this would not have
been possible, since \easycrypt\ can only reason about indistinguishability of
sampled values at the time of sampling.
The reason for this, is that random values are not expressed as random values in
\easycrypt. Instead, when sampling a random value we change the output
distribution to quantify over all values, which can be sampled from the
distribution. What the coupling functionality, in essence, does is that it allow
us to fix a specific value for the randomly sampled value if we can prove the
value we fix is indistinguishable from a randomly sampled value.

In other words, when we sample a random values we introduce a new value where
the only constraint on the value is that it must sampled from the distribution.
However, \easycrypt\ offers no functionality for manipulating the value after it
has been sampled, since if we could just change the value we could potentially
prove contractionary statements.
Consider the same where we sample a random boolean, and then check the value of
it. By the coupling functionality we could fix the value to be the negation of
the sampled value. If we could change the randomly value again we could
change it back to the original value, thus arriving at a contradiction where the
variable is both true and false.
...
\begin{proof}[Proof of lemma \ref{lem:zkboo:decomposition:privacy}]
  ...
  \]
  Then we have
  $(y^{real}_{e} + y^{real}_{e+1}) \sim (y^{simulated}_{e} + y^{simulated}_{e+1})$
  since the views of the two procedures are indistinguishable. We can then conclude:
  \begin{align*}
...
Having formalised both arithmetic circuits and their (2,3)-Decomposition
we are now ready to formalise the ZKBoo protocol.
Since ZKBoo is a $\Sigma$-Protocol we start by
defining its types as specified in section \ref{ch:formal_sigma}, which can be
seen in \autoref{lst:zkboo:types}.

\begin{lstlisting}[mathescape, float, label=lst:zkboo:types, caption=Type definitions for ZKBoo]
...
The relation is then all tuples of (circuits, outputs, inputs), for which it holds
that evaluating the circuit with the input returns the output. We formalise this as:
\begin{equation}
  \text{R } = \{((c,y), w) \,|\, \texttt{eval\_circuit } c \; w = y\}.
\end{equation}

We then define the functions needed to verify the views from the decomposition,
as outlined by the verification step in section \ref{subsec:general:zkboo}.
...
which are useful to reason about in pre- and postcondition of procedures.
...
perfectly legal, but not possible to express as a computation
...
bounds the number of computations. We can, therefore, define a computable function which checks
for each entry in the circuit if the property holds and then returns if the property held
...
To achieve this we introduce a specialized version of a reflection lemma proven by \cite{Yao},
...
implementing the algorithms from \autoref{lst:sigma_procedures}, which can be
seen in \autoref{lst:zkboo_procedures}.
...
proc init(h : statement, w : witness) = {
  ...
  $y_{2} = $ last $w_{2}$;
  $y_{3} = $ last $w_{3}$;
  return $(y_{1}, y_{2}, y_{3}, c_{1}, c_{2}, c_{3})$;
}
...
proc verify(h : statement, m : message, e : challenge, z : response) = {
  $(y_{1}, y_{2}, y_{3}, c_{1}, c_{2}, c_{3})$ = m;
  (c, y) = h;

  $(k'_{e}, w'_{e}, k'_{e+1}, w'_{e+1})$ = z;
  valid_com1 = Com.verify $(w'_{e}, k'_{e})$ $c_{e}$;
  valid_com2 = Com.verify $(w'_{e+1}, k'_{e+1})$ $c_{e+1}$;
  valid_share1 = last $w'_{e}$   = $y_{e}$;
  valid_share2 = last $w'_{e+1}$ = $y_{e+1}$;
  valid = valid_view_op 1 $w'_{e}$ $w'_{e+1}$ c $k'_e$ $k'_{e+1}$;
  valid_length = size c = size $w'_e - 1$ /\ size $w'_{e}$ = size $w'_{e+1}$;

  return $y = y_1 + y_2 + y_3$ /\ valid_com1 /\ valid_com2 /\ valid_share1 /\ valid_share2 /\ valid /\ valid_length
}
...
Given that ZKBoo is a $\Sigma$-Protocol we simply need
...
assume Com satisfies the perfect hiding definition
...
game for commitment schemes defined in chapter \ref{ch:formal_commitment}.

We, therefore, replace the calls to the
commitment procedures with the correctness game which can be seen in
\autoref{lst:zkboo-inter-completeness}.
To do so, we need to swap the
...
do so. If, however, \texttt{Com.verify} had been a procedure we could not change
the order of the calls,
since one verification could potentially change the state of another.

\begin{lstlisting}[mathescape, label=lst:zkboo-inter-completeness,caption= Intermediate game for completeness, float]
...
Next, since the commitment scheme and the decomposition is correct we left
...
\begin{proof}
  ...
  we only show the case of $e=1$. When $e=1$ the two procedures
  are:
...
proc ideal(h, e) = {
  ...
  $x_e$ <$\$$ dinput;
  $x_{e+1}$ <$\$$ dinput;
  $(k_{e}, k_{e+1}, w_{e}, w_{e+1}, y_{e+2})$ = simulated$(c, [x_{e}], [x_{e+1}])$;

  (* Generate random list of shares *)
  $w_{e+2}$ = dlist dinput (size $w_{e}$);
  $k_{e+2}$ = dlist dinput (size $k_{e}$);
  $y_{e}$ = last $w_{e}$;
  $y_{e+1}$ = last $w_{e+1}$;
  $c_{1}$ = Commit($(w_{1}, k_{1})$);
  $c_{2}$ = Commit($(w_{2}, k_{2})$);
  $c_{3}$ = Commit($(w_{3}, k_{3})$);
  a = $(y_{1}, y_{2}, y_{3}, c_{1}, c_{2}, c_{3})$;
  z = $(k_{e}, w_{e}, k_{e+1}, w_{e+1})$;
  ...
  From the 2-Privacy of the decomposition we have that \texttt{compute} and
  \texttt{simulate} are indistinguishable, but only when observing the views
  $w_{e}, w_{e+1}$ and output share $y_{e+2}$. By this indistinguishability we
  can then apply the correctness lemma to both sides, thus making both procedures
  return true.

  We, therefore, need to argue that $c^{ideal}_{e+2} \sim c^{real}_{e+2}$.
  In the real case $c_{e+2}$ is a commitment to
  the view produced by the decomposition.
  ...
  (\ref{def:commitment:perfect-hiding}) to step both procedures forward.
...
If we were using the original hiding definition based on an adversary (definition \ref{def:commitment:hiding}),
...
An interesting property of this dilemma is that the alternative definition actually
implies the original definition. Given the proof of procedure
indistinguishability we can use it to rewrite the adversarial game to the
adversary comparing two commitments to randomly chosen messages, which there is
no way for the adversary to distinguish.
The other direction, however, is not clear. Intuitively, the adversarial game
should also imply the pRHL statement since if there is no way for any adversary
to distinguish them, then it must be because the output of the procedures are
indistinguishable. However, since there is no way to using statements
of adversarial indistinguishability to substitute procedures calls with each
other in \easycrypt\ we cannot directly use the adversarial game to prove the
pRHL statement.

\vspace{3mm}
\noindent Lastly, we want to prove the 3-special soundness property of ZKBoo. To do so, we
first inline the procedures of the soundness game. We then group the procedures
into a procedure, \texttt{extract\_views}, which verifies that the views
are accepting. To verify the views it uses the binding game, which also checks
that the responses does not break the binding property.
This is seen in \autoref{lst:zkboo:alt-soundness}

\begin{lstlisting}[float,label=lst:zkboo:alt-soundness,caption=Soundness game for ZKBoo]
...
change the output distribution of the soundness game since the output
distributions of verifying a commitment and the alternative binding game are identical.
...
For each view we have there are two potential openings $w_{i}$ and
$w'_{i}$. Now, since the two potential openings corresponds to a commitment in
message $a$, it should hold that $w_{i} = w'_{i}$. Otherwise, the prover has
managed to break the binding property of the commitment scheme.
When $\forall i. w_{i} = w'_{i}$ we refer to the responses as begin \textit{consistent}.
...
\end{lemma}
\begin{proof}
  This follows directly from applying our assumptions.
\end{proof}

Consequently, since \texttt{extract\_views} returns true we can conclude \validviews{c, w_1, w_2, w_3} and
$\sum_{i \in \{1,2,3\}} \text{last } w_{i} = y$

Next, we prove that if the responses are consistent then
...
\begin{proof}
  ...
  the while-loop it will perform exactly the same computations as the ones
  ...
  produce the correct output $y$; hence summing the input shares must be a valid witness for the
  relation.
\end{proof}
