...
The above notation of parametrising modules does encapsulate
...
not type check, since we have no way of knowing that the types of $S_{1}$ and
$S_{2}$ will be.
...
\begin{proof}
  ...
  \texttt{Completeness(ANDProtocol).special} we see that it is equivalent first
  ...
  We need to use the \texttt{special} definition of completeness
  since the challenge $e$ is given by a Verifier running the AND construction.
  Were we to use the original definition of completeness, then the AND construction
...
For the OR construction we use the definition by \citet{on-sigma-protocols},
