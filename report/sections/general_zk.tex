...
In previously section, we have seen a concrete instantiation of a
...
However, since the need for zero-knowledge is wide spread, it is unlikely that a
...
However, the user is not willing to reveal his birthday. To solve this problem
we could define a $\Sigma$ protocol for proving knowledge of the birthday making
...
To solve this problem, we could either pre-determine all the relations we could
need zero-knowledge proofs of. However, another approach is that of generalised
...
input to some function, which results in to a specific output.

To explore the field of generalised zero-knowledge protocol we introduce ZKBoo
and its security definition by \citet{zkboo}.
...
These two facts contribute the ``MPC in the head'' approach to be significantly
...
Then, if we reveal $d$ of the $n$ computation branches to a verifier, he
...
which is a ``MPC in the head'' protocol for splitting the evaluation of a
circuit into three branches with $2$-privacy and correctness. We then introduce
the actual ZKBoo protocol, and describe how the views of a MPC protocol can be
...
Throughout this section we will simply refer the (2,3)-Function decomposition
of a function $f$ as $\decomp_{f}$.

We refer to the three parties of the decomposition as $P_{1}, P_{2}, P_{3}$. The
decomposition then works by splitting the input into a input share for each
party, where the original input can be obtain if all three input shares are
acquired.
Each party then evaluates all the gates of the circuit to a value, as described by
the MPC protocol. Here, we mimic communication of the parties by simply letting
them access each others views. As a general rule we only allow party $P_{i}$ to
communicate with party $P_{i+1 \mod 3}$.
The view of a party is then a list  of all the values that
the party has computed so far.
...
From this protocol we can see that if $\decomp_{C_{f}}$ is correct and we get access to
